              ...
              <div className="rounded-[2rem] bg-white p-6 shadow-sm">
                ...
                <div className="rounded-2xl border border-slate-100">
                  <table className="w-full table-fixed text-left text-[11px]">
                    <colgroup><col className="w-[30%]" /><col className="w-[18%]" /><col className="w-[19%]" /><col className="w-[19%]" /><col className="w-[10%]" /><col className="w-[4%]" /></colgroup>
                    <thead className="bg-slate-50 text-[9px] uppercase tracking-wide text-slate-500"><tr><th className="px-2 py-2 font-bold">Produk</th><th className="px-2 py-2 font-bold">Kategori</th><th className="px-2 py-2 font-bold">Jual</th><th className="px-2 py-2 font-bold">Modal</th><th className="px-2 py-2 font-bold">Qty</th><th className="px-1 py-2" /></tr></thead>
                    <tbody>{products.map(product => <tr key={product.id} className="border-t border-slate-100"><td className="px-2 py-2"><div className="flex min-w-0 items-center gap-1.5"><div className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg bg-gradient-to-br ${product.color}`}><span className="text-[10px] font-black text-slate-500">{product.name.charAt(0)}</span></div><span className="truncate font-bold">{product.name}</span></div></td><td className="truncate px-2 py-2 text-slate-500">{product.category}</td><td className="px-2 py-2"><input type="number" min="0" value={product.price} onChange={event => updateProduct(product.id, "price", event.target.value)} className="w-full min-w-0 border-0 bg-transparent p-0 font-semibold outline-none focus:ring-0" /></td><td className="px-2 py-2"><input type="number" min="0" value={product.costPrice} onChange={event => updateProduct(product.id, "costPrice", event.target.value)} className="w-full min-w-0 border-0 bg-transparent p-0 font-semibold outline-none focus:ring-0" /></td><td className="px-2 py-2"><input type="number" min="0" step="1" value={product.stock} onChange={event => updateProduct(product.id, "stock", event.target.value)} className="w-full min-w-0 border-0 bg-transparent p-0 font-semibold outline-none focus:ring-0" /></td><td className="px-1 py-2 text-right">{!initialProducts.some(item => item.id === product.id) && <button onClick={() => setProducts(current => current.filter(item => item.id !== product.id))} className="text-slate-300 hover:text-rose-500" aria-label={`Hapus ${product.name}`}><Trash2 size={14} /></button>}</td></tr>)}</tbody>
                  </table>
                </div>
              </div>
            </section>}
