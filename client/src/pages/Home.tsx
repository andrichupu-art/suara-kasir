              ...
              <div className="rounded-[2rem] bg-white p-6 shadow-sm">
                ...
                <div className="overflow-x-auto rounded-2xl border border-slate-100">
                  <table className="w-full min-w-[620px] text-left text-xs">
                    <thead className="bg-slate-50 text-[11px] uppercase tracking-wider text-slate-500"><tr><th className="px-3 py-3 font-bold">Produk</th><th className="px-3 py-3 font-bold">Kategori</th><th className="px-3 py-3 font-bold">Harga jual</th><th className="px-3 py-3 font-bold">Harga modal</th><th className="px-3 py-3 font-bold">Qty</th><th className="px-3 py-3" /></tr></thead>
                    <tbody>{products.map(product => <tr key={product.id} className="border-t border-slate-100"><td className="px-3 py-3"><div className="flex items-center gap-2"><div className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-gradient-to-br ${product.color}`}><span className="font-black text-slate-500">{product.name.charAt(0)}</span></div><span className="whitespace-nowrap font-bold">{product.name}</span></div></td><td className="px-3 py-3 text-slate-500">{product.category}</td><td className="px-3 py-3"><input type="number" min="0" value={product.price} onChange={event => updateProduct(product.id, "price", event.target.value)} className="w-24 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 font-semibold outline-none focus:border-emerald-400" /></td><td className="px-3 py-3"><input type="number" min="0" value={product.costPrice} onChange={event => updateProduct(product.id, "costPrice", event.target.value)} className="w-24 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 font-semibold outline-none focus:border-emerald-400" /></td><td className="px-3 py-3"><input type="number" min="0" step="1" value={product.stock} onChange={event => updateProduct(product.id, "stock", event.target.value)} className="w-20 rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5 font-semibold outline-none focus:border-emerald-400" /></td><td className="px-3 py-3 text-right">{!initialProducts.some(item => item.id === product.id) && <button onClick={() => setProducts(current => current.filter(item => item.id !== product.id))} className="text-slate-300 hover:text-rose-500" aria-label={`Hapus ${product.name}`}><Trash2 size={16} /></button>}</td></tr>)}</tbody>
                  </table>
                </div>
              </div>
            </section>}
