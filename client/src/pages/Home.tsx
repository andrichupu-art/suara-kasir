        ...
        <nav className="fixed bottom-3 left-1/2 z-20 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 lg:hidden">
          <div className="relative flex h-[74px] items-center justify-between rounded-[2rem] border border-slate-100 bg-white/95 px-6 shadow-[0_12px_32px_rgba(15,23,42,0.14)] backdrop-blur">
            ...
            <div className="relative z-10 flex w-[44%] justify-between">
              {nav.slice(0, 2).map(item => <button key={item.id} onClick={() => { setReportDate(null); setActiveTab(item.id); }} aria-label={item.label} className={`relative grid h-12 w-12 place-items-center rounded-2xl transition ${item.id === "produk" ? "-translate-x-10" : ""} ${activeTab === item.id ? "text-emerald-600" : "text-slate-300 hover:text-slate-500"}`}><item.icon size={22} />{activeTab === item.id && <span className="absolute -bottom-1 h-1.5 w-1.5 rounded-full bg-emerald-500" />}</button>)}
            </div>
            ...
            <div className="relative z-10 flex w-[44%] justify-between">
              {nav.slice(2).map(item => <button key={item.id} onClick={() => { setReportDate(null); setActiveTab(item.id); }} aria-label={item.label} className={`relative grid h-12 w-12 place-items-center rounded-2xl transition ${item.id === "riwayat" ? "translate-x-10" : ""} ${activeTab === item.id ? "text-emerald-600" : "text-slate-300 hover:text-slate-500"}`}><item.icon size={22} />{activeTab === item.id && <span className="absolute -bottom-1 h-1.5 w-1.5 rounded-full bg-emerald-500" />}</button>)}
            </div>
          </div>
        </nav>
      </div>
