...
function normalizeCommand(raw: unknown) {
  ...
  const result = z.object({
    action: z.enum(["add_item", "remove_item", "stock_low", "stock_safe", "add", "remove", "checkout", "cancel", "summary", "unknown"]).default("unknown"),
    type: z.enum(["add", "remove", "stock_low", "stock_safe", "checkout", "cancel", "summary", "unknown"]).optional(),
  ...
  return { ...result.data, action: result.data.type === "add" ? "add_item" : result.data.type === "remove" ? "remove_item" : result.data.type === "stock_low" ? "stock_low" : result.data.type === "stock_safe" ? "stock_safe" : result.data.type === "summary" ? "summary" : action };
}
